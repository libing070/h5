...
export default {
	...
	methods: {
		...
		getFawvwmaterialTypeList: function() {
			var that = this;
			this.get(this.base + "/api/fawvwmaterial/type/list", null, function(data){
				if (data.code==200) {
		...
		submit: function() {
			...
			this.json(this.base + "/api/fawvwmaterial/submit", this.cc(params), function(data){
				if (data.code==200) {
					that.showMsg("提交成功");
					history.go(-1);
				}else{
		...
		parentHeight: function() {
			$(window.parent.document).find("iframe").height(($(".t-list-edit").height()+400)+'px');
		},
	}
}
</script>
