<!--问卷调查问卷未提交列表-->
<template>
<div class="w-list-unanswer" v-cloak>
	<div class="list">
		<table>
			<thead>
				<tr>
			...
			<tbody>
				<tr v-for="(item,index) in data.list">
					...
					<td v-if="item.status=='收集中'"><span @click="edit(item.id)">答题</span></td>
					<td v-if="item.status=='已结束'"><span></span></td>
					<td v-if="item.status=='未开始'"><span></span></td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	export default {
		...
		methods: {
			getData: function() {
				...
				this.get(this.base + "/api/policy/list/unanswer", params, function(data){
					if (data.code==200) {
						that.data = data.data;
			...
			edit: function(id) {
				window.location = "/list_w_wenjuan?qaId="+id;
			},
