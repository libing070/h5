...
$(function () {
    ...
    $("#tiaokuan").css("visibility", "visible");
...
$(function () {
    ...
    function getJxsDefult(cityName) {
        var curUtm=getUtms();
        var parame = {
            url: '/api/index.php?c=home&a=getjxss&format=json&city=' + cityName+'&utm='+curUtm
        };
        $.getDistri(parame, function (res) {
            var str = '';
            res.items.forEach((val, key) => {
    ...
    function getInquiryJxs(proName, cityName, searchVal, load_num) {
        ...
        var curUtm=getUtms();
        var parame = {
            url: '/api/index.php?c=home&a=getjxss&format=json&q=q&webtype=mobile&che=' + cheName + '&q=1&utm=' + curUtm + '&_keyword=' + searchVal + '&city=' + pro_city
        };
        $.getDistri(parame, function (res) {
            var str = '';
            res.items.forEach((val, key) => {
            ...
            success: function (res) {
                ...
                try {
                    stm_clicki('send', 'event', {
                        ...
                        customActionLabel3: $('.form-div').eq(2).find('option:selected').html()+'/'+$('.form-div').eq(3).find('option:selected').html(),
                        customActionLabel4: $('.form-div').eq(4).find('option:selected').html(),
                        customActionLabel5: utms
                    });
