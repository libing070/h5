...
iSlider.prototype={
    ...
    _bindEvt:function () {
        ...
        handlrElm.addEventListener('touchstart',startFun,false);
        startMoveFun=function(e){
            self._touchmove(e);
        ...
        handlrElm.addEventListener('touchmove',startMoveFun,false);
        startEndFun=function(e){
            self._touchend(e);
        };
        handlrElm.addEventListener('touchend',startEndFun,false);
        startCancelFun=function(e){
            self._touchend(e);
        };
        handlrElm.addEventListener('touchcancel',startCancelFun,false);
        startOtherFun= function (e) { e.preventDefault(); }
        if (this.opts.fullScr || this.opts.preventMove) {
            handlrElm.addEventListener('touchmove', startOtherFun, false);
        }
    },
...
//                console.info(e)
    ...
    gandiao:function(){
        ...
        var handlrElm= this.opts.fullScr ? this.$('body') : this.wrap;
        if(startFun){
            handlrElm.removeEventListener("touchstart", startFun);
        }
        if(startFun){
            handlrElm.removeEventListener("touchmove", startMoveFun);
        }
        if(startFun){
            handlrElm.removeEventListener("touchend", startEndFun);
        }
        if(startFun){
            handlrElm.removeEventListener("touchcancel", startCancelFun);
        }
        if(startFun){
            handlrElm.removeEventListener("touchmove", startOtherFun);
        }
    },
    /**
     * 滑动到上一页
