...
$(function () {
    ...
    $.share({
        title: '奥迪Q3轿跑 觉醒亮相',
        desc: '超感观美学豪华A级SUV奥迪Q3轿跑觉醒上市，让感观即刻被重新焕发！',
...
$(".inquiryPrice").on("mousedown",'.app-fun',function () {
        ...
        $.sendData(data, function (res) {
            if (res.msg.indexOf('成功') > -1) {
                layer.msg('预约成功', function () {
                    _pt_sp_2.push('setCustomEvent',{eventName:'q3 wap 预约成功'});
                    try {
   ...
   // 打开发现媒体
    ...
    $('.yuyue').on('click', function () {
        $(".page1").show();
        $(".menu").show();
        $(".menu").find(".s1").eq(0).addClass("active");
        $(".menu").find(".s1").eq(1).removeClass("active");
        $(".return").hide();
        $(".page2").hide();
        $(".page3").hide();
        resetVideo();
        $('.evaluation_pop .closeIframe').click();
        $('html, body').animate({
            scrollTop: $(".liuzi").offset().top
        }, 500);
    })
        ...
        $.getResultByParams({
           ...
           //console.log(res);
            res.items=randomSort(res.items);
                ...
                var distriStr = '';
                res.items.forEach(function(val, i){
                    if(i%2==0){
                        distriStr += '<div style="background-color: #272727" class="list">' ;
                    }else{
                        distriStr += '<div style="background-color: #353535" class="list">' ;
                    }
                    distriStr +=    '<h3>' + val.gongsi + '</h3>' ;
                    distriStr +=   '<p><span class="icon dizhi"></span><span>' + val.dizhi + '</span></p>' ;
                    distriStr +=    '<p><span class="icon dianhua"></span><span>' + val.dianhua + '</span></p>';
                    distriStr +=   '<div class="btns">' ;
                    distriStr +=   '<a  jingxiaoshangid="'+val.jingxiaoshangid+'" class="app_now2 app-fun" vid="' + i + '" belong_id="' + val.id + '">立即预约</a>' ;
                    distriStr +=   '<a class="call_now" href="tel:' + val.tel + '">一键呼叫</a>' ;
                    distriStr +=   '</div>';
                    distriStr +=   '</div>';
                });
