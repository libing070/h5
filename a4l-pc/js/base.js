...
$("#wenda").on('click',function(){
    window.open('https://ai.auto-smart.com/audi_a6l/robot.html');
})
...
var time_range = function (beginTime, endTime) {
    ...
    var currSysTime = new Date();
    var my_hours=currSysTime.getHours();
    var my_minutes=currSysTime.getMinutes();
    var nowTime=my_hours+":"+my_minutes;
   // var nowTime="23:30";
    ...
    if (n.getTime () - b.getTime () >= 0 && n.getTime () - e.getTime () <= 0) {
        layer.msg("当前时间是：" + n.getHours () + ":" + n.getMinutes () + "，正在直播！");
        return true;
    }else if(n.getTime () - b.getTime () < 0){
        layer.msg ("当前时间是：" + n.getHours () + ":" + n.getMinutes () + "，未开始直播！");
        return false;
    }else if(n.getTime () - e.getTime () > 0){
        layer.msg ("当前时间是：" + n.getHours () + ":" + n.getMinutes () + "，直播已经结束！");
        return false;
    }
    else {
        layer.msg ("当前时间是：" + n.getHours () + ":" + n.getMinutes () + "，不在该时间范围内！");
        return false;
    }
}
//time_range ("21:30", "23:30");

$(".live-class").on("click",'.livebtn',function () {
    ...
    if(link=="zhihu"){
        layer.open({
            ...
            title:'   ',
            area: ['300px', '300px'], //宽高
            content: '<div id="qrcode" style="width:200px; height:200px;margin: auto; margin-top:15px;"><img src="./images/zhihuRcode.png"></div><div style="margin-top: 10px;font-size: 14px;text-align: center;padding: 0 50px">打开微信，扫码进入。</div>'
        });
    }else if(link=="tencent"){
        layer.open({
            type: 1,
            title:'   ',
            area: ['300px', '300px'], //宽高
            content: '<div id="qrcode" style="width:200px; height:200px;margin: auto; margin-top:15px;"><img src="./images/tencentRQcode.png"></div><div style="margin-top: 10px;font-size: 14px;text-align: center;padding: 0 50px">打开微信，扫码进入。</div>'
        });
    }else if(link=="jxs"){
        layer.open({
        type: 1,
        title:'   ',
        area: ['300px', '300px'], //宽高
        content: '<div id="qrcode" style="width:200px; height:200px;margin: auto; margin-top:15px;"><img src="./images/jsxRcode.png"></div><div style="margin-top: 10px;font-size: 14px;text-align: center;padding: 0 50px">打开微信，扫码进入。</div>'
    });
    }else{
