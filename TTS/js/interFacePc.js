            ...
            function (res) {
                ...
                if(res.data.articleList.length>0){
                    res.data.articleList.forEach((ele, k) => {
                        ...
                        str +='<a href="./details.html?id='+ele.id+'"><img style="width: 100%;height: 2.4rem" src="'+url+'"></a>';
                ...
                //知识
                var str2 = '';
                if(res.data.knowHowList.length>0){
                    res.data.knowHowList.forEach((ele, k) => {
                        str2 +='<div class="swiper-slide box">';
                        var url= '';
                        NetPing(ele.picUrl,function (res) {
                            if(!res){
                                url='./images/404.png'
                        ...
                        str2 +='<a href="./details.html?id='+ele.id+'"><img style="width: 100%;height: 2.4rem" src="'+url+'"></a>';
                        str2 +='<div class="content">';
                        str2 +='<p class="time">'+ele.createTime+'</p>';
                        str2 +='<p class="title">'+ele.title+'</p>';
                        str2 +='<p class="desc">'+ele.summary+'</p>';
                        str2 +='</div>';
                        str2 +='<p class="bottom">';
                        str2 +='<span class="item"><img src="./images/message-icon.png"><span >'+ele.commentCount+'</span></span>';
                        str2 +='<span class="item"><img src="./images/view-icon.png"><span>'+ele.visitCount+'</span></span>';
                        str2 +='<span class="item"><img forumId="'+ele.id+'" class="vote-icon-img" src="'+(ele.IsLike?"./images/vote-icon-red.png":"./images/vote-icon.png")+'"><span>'+ele.likeCount+'</span></span>';
                        str2 +='</p>';
                        str2 +='</div>';
                    });
                    $('.index-banner4 .swiper-wrapper').append(str2);
                }


                //热门活动
                var str3='';
                if(res.data.hotList.length>0){
                    res.data.hotList.forEach((ele, k) => {
                        ...
                        str3+='<img class="click-img" forumId="'+ele.id+'"  style="width: 100%;cursor: pointer;height: 1.8rem" src="'+url+'">';
                ...
                for(var i=0;i<recommendList.length;i++){
                     if(i==0){
                         ...
                         str4+='  <img class="click-img" style="width: 100%;cursor: pointer;height: 2.4rem"  forumId="'+recommendList[i].id+'" src="'+url+'">';
