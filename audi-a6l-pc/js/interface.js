...
jQuery.extend({
     ...
     */
    share: function (cfgs, param) {
        var wxInfo = {};
        var locaH = window.location.href;
        var lastS = locaH.lastIndexOf('/');
        var url = locaH.substring(0, lastS);
        var k_host = document.domain;
        var cfg = {
            share_url: locaH,
            title: cfgs.title,
            desc: cfgs.desc,
            imgUrl: "http://" + k_host + cfgs.imgUrl
        };
        $.ajax({
            type: param.type,
            url: host + param.url + '&timestamp=' + new date().getTime(),
    ...
    intelligence: function (obj, isVR) {
        var url = '';
        if (isVR && isVR == 'VR') {
            url = 'https://www1.pcauto.com.cn/appearance/260.html'
        } else {
