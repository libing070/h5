(function ($) {
    $.fn.drag = function (options, sucfun, errfun) {
        ...
        // console.log(clientWidth * 0.64, handler.width())
        var maxWidth = clientWidth * 0.64 - handler.width();  //能滑动的最大间距
        //鼠标按下时候的x轴的位置
        ...
        $("#drag").on('touchend', function (e) {
            ...
            isMove = false;
            var leftPx=$("#xy_img").position().left;
            var xPoint=leftPx/perBli;
            console.log("xPoint:"+xPoint);
            $.ajax({
                url: url + "/api/checkReqCode",
                type: "get",
        ...
        //清空事件
        function dragOk(_x) {
            $(".drag_bg").css("width",clientWidth * 0.64+"px");
            $(".handler_bg").css("left",maxWidth+"px");
